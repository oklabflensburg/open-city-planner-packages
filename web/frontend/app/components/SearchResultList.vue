  ...
  <ul role="listbox" class="divide-y divide-slate-100">
    <li v-for="(pkg, index) in items" :key="pkg.id">
      <button
        ...
        @click="emit('choose', pkg)"
      >
        <PackageIcon :name="pkg.name" compact />
        <span class="min-w-0">
          <span class="flex items-center gap-2"
            ><strong class="truncate">{{ pkg.name }}</strong
            ><PackageBadge
              :value="pkg.channels.stable ? 'stable' : 'Kein Stable-Channel'"
          /></span>
          <span class="mt-0.5 block truncate font-mono text-xs text-slate-500"
            >{{ pkg.id }} · {{ pkg.publisher.name }}</span
          >
        ...
        <span class="text-right"
          ><code class="text-xs font-semibold">{{
            pkg.channels.stable?.version || 'Keine Stable-Version'
          }}</code
          ><span class="mt-1 block text-[11px] text-slate-500">{{
            pkg.classification === 'first-party' ? 'First Party' : 'Community'
          }}</span></span
        >
      </button>
    </li>
